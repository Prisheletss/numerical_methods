...
+ [реальный маятник (не доделанно)](#реальный-маятник)
...
На тело действуют сила тяжести $\bar{f}_{тяж} = m\bar{g}$ и сила аэродиномического сопротивления $\bar{f}_{сопр}=-\alpha \bar{v}$. Запишем второй закон ньютона:
...
На конец маятника действуют силы $m\bar{g}$, $\bar{T}$, $\bar{f}_{сопр}$, где $\bar{f}_{сопр} = -\alpha\bar{v}$ 
