...
+ [реальный маятник](#реальный-маятник)
+ идеальный газ (не доделанно)


# Задача двух тел
## Описание задачи и решение
![image](images/гравитация.png)
...
На тело действуют сила тяжести $\bar{f} = m\bar{g}$ и сила аэродиномического сопротивления $\bar{f} = -\alpha \bar{v}$. Запишем второй закон ньютона:
...
![image](images/fly_calculations.png)
...
Рассмотрим маятник, состоящий из тонкого невесомого нерастяжимого и несжимаемого стержня длины L, на конце которого закрепленна точечная масса m. маятник отводят из положения равновесия на большой угол $\varphi_0$, и запускают с начальной скоростью $v_0$. 

На конец маятника действуют силы $m\bar{g}$, $\bar{T}$, $\bar{f}_{сопр} = -\alpha\bar{v}$

Моменты сил (в проекции на ось вращения): $M_g = -mgL*sin(\varphi)$, $M_T = 0$, $M_с = -\alpha L^2\dot{\varphi}^2$ 

Запишем основное уравнение динамики вращательного движения: $M_r = J\varepsilon$ \
Получим: 

$$-mgL*sin(\varphi) -\alpha L^2\dot{\varphi}^2 = mL^2\ddot{\varphi}$$

Производя замену $A = \frac{\alpha}{m}$, $B = \frac{g}{L}$ -- получим диффур:

$$\ddot{\varphi} + A\dot{\varphi} + Bsin(\varphi) = 0$$
...
$$\frac{\varphi(t+\delta) - 2\varphi(t) + \varphi(t-\delta)}{\delta^2} + A\frac{\varphi(t+\delta) - \varphi(t)}{\delta} + Bsin(\varphi(t)) = 0$$

Решая, получим:

$$\varphi(t+\delta) = \frac{\varphi(t)\cdot(2+A\delta) - B\delta^2\cdot sin(\varphi(t)) - \varphi(t-\delta)}{1+A\delta}$$

![image](images/phase_diagram.png)

Не столь интересно анализировать координаты, сколько интересно анализировать фазовую диаграмму -- поэтому именно её и запишем

Несколько вариантов движения:
+ Оранжевый -- без сопротивления при большой начальной скорости
+ Синий -- с большой начальной скорости при некотором сопротивлении
+ Серый -- сепаратриса (с центром около положения равновесия синего)
+ Жёлтый -- движение, близкое к сепаратрисе
...
```
В файл `cords.txt` сохраняются пары угол-угловая скорость
...
f = f_1 + w*d;
```
+ `f_1` -- предыдущий угол отклонения маятника от вертикальной оси
+ `f` -- текущий угол отклонения маятника от вертикальной оси
+ `f_2` -- хендлер для нового угла
+ `w` -- начальная угловая скорость
+ `d` -- шаг симуляции
+ `A` -- коэффициент аэродинамического сопротивления
+ `B` -- коэффициент инерциальности
...
```
В цикле на протяжении 2500 итераций производятся вычисления, описанные выше